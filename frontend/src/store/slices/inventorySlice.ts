...
export const placeStructure = createAsyncThunk(
  ...
  async (
    { userId, structureName, plotIndex }: { userId: string; structureName: string; plotIndex: number },
    { rejectWithValue }
  ) => {
    try {
...
const inventorySlice = createSlice({
  ...
  reducers: {
    ...
    optimisticStructureUse: (state, action: PayloadAction<{ structureName: string; count: number }>) => {
      const { structureName, count } = action.payload
      state.optimisticUpdates[structureName] = (state.optimisticUpdates[structureName] || 0) + count
    },

...
export type PomoBlockPhase = "idle" | "work";
...
export const updatePomoBank = createAsyncThunk(
  ...
  async ({ userId, minutes }: { userId: string; minutes: number }, { rejectWithValue }) => {
    try {
...
const timerSlice = createSlice({
  ...
  reducers: {
    ...
    completePhase: (state) => {
      if (state.hasProcessedCompletion) {
      ...
      if (state.currentPhase === "work") {
        ...
        // Calculate earnings for this work session

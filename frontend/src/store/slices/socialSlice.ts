...
export const addFriend = createAsyncThunk(
  'social/addFriend',
  async (
    { userId, friendEmail }: { userId: string; friendEmail: string },
    { rejectWithValue }
  ) => {
    try {
...
export const createGroup = createAsyncThunk(
  'social/createGroup',
  async (
    { userId, groupName }: { userId: string; groupName: string },
    { rejectWithValue }
  ) => {
    try {
...
export const joinGroup = createAsyncThunk(
  'social/joinGroup',
  async (
    { userId, groupCode }: { userId: string; groupCode: string },
    { rejectWithValue }
  ) => {
    try {
...
const socialSlice = createSlice({
  ...
  reducers: {
    ...
    updateFriendStatus: (state, action: PayloadAction<{ friendId: string; status: 'pending' | 'accepted' }>) => {
    ...
    updateGroupMembership: (state, action: PayloadAction<{ groupId: string; memberIds: string[] }>) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(createGroup.fulfilled, (state, action) => {
        ...
        const optimisticIndex = state.groups.findIndex(g => g.group_name === newGroup.group_name && !g.group_id)

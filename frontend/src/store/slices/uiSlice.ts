...
const uiSlice = createSlice({
  ...
  reducers: {
    ...
    // Tab management
    setActiveTab: (state, action: PayloadAction<{ section: keyof UIState['activeTabs']; tab: string }>) => {
    ...
    openStructuresModal: (state, action: PayloadAction<{ plotIndex?: number }>) => {
      state.modals.structures = true
      if (action.payload.plotIndex !== undefined) {
        state.temp.selectedPlotIndex = action.payload.plotIndex
      }
    },
  },
})

...
interface BaseModalProps {
  ...
  className?: string;
  zIndex?: number;
  constrainToCanvas?: boolean; // New prop to constrain modal to canvas area
}
...
export default function BaseModal({
  ...
  className = "",
  zIndex = 1000,
  constrainToCanvas = false, // Default to false for backward compatibility
}: BaseModalProps) {
  ...
  return (
    ...
    >
      <div style={{ padding: "20px" }}>
        <p style={{ color: FONTCOLOR }}>Modal content goes here.</p>
        <p style={{ color: FONTCOLOR, fontSize: "14px", opacity: 0.7 }}>User ID: {userId}</p>
      </div>
    </BaseModal>
  );
}

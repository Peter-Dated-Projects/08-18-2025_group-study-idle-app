...
def add_finished_tutorial_field_to_users():
    ...
    try:
        ...
        RETURN length
        """
        existing_finished_tutorial_count = list(
            db.aql.execute(existing_finished_tutorial_query)
        )[0]
        logger.info(
            f"Users already with finished-tutorial field: {existing_finished_tutorial_count}"
        )
        ...
        logger.info(
            f"Successfully updated {len(updated_users)} users with finished-tutorial=false"
        )
        ...
        logger.info(
            f"Verification: {final_count} users now have finished-tutorial field"
        )
        ...
        if updated_users:
            ...
            for i, user in enumerate(updated_users[:5]):
                logger.info(
                    f"  {i+1}. User {user.get('user_id', user.get('_key'))}: finished-tutorial={user.get('finished-tutorial')}"
                )
        ...
        if final_count == total_users:
            ...
            logger.warning(
                f"⚠️  WARNING: Expected {total_users} users with finished-tutorial field, but found {final_count}"
            )
            return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not client.ping():
            logger.error(
                "❌ Cannot connect to ArangoDB. Please check your connection settings."
            )
            sys.exit(1)

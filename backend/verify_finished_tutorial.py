...
def verify_finished_tutorial():
    ...
    for i, user in enumerate(results, 1):
        ...
        print(
            f"{status} {i}. {user['user_id']}: finished-tutorial = {user.get('finished-tutorial', 'MISSING')}"
        )
    ...
    print("\n" + "=" * 60)
    print(
        f"Summary: {users_with_field}/{len(results)} users have the finished-tutorial field"
    )
    print("=" * 60)

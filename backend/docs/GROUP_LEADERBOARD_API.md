...
leaderboard.leaderboard.forEach(entry => {
  console.log(`${entry.group_rank}. ${entry.user_id}: ${entry.score} pomos (Global rank: ${entry.rank})`);
});
```
...
const response = await fetch(`/api/group-leaderboard/compare-groups?group_ids=${groupIds}&period=monthly`);
const comparison = await response.json();

// Show group competition results
comparison.groups.forEach(group => {
  console.log(`${group.rank}. ${group.group_name}: ${group.total_score} total (${group.average_score} avg)`);
});
```
...
userGroups.group_rankings.forEach(group => {
  console.log(`${group.group_name}: Weekly rank ${group.ranks.weekly.rank}/${group.ranks.weekly.total_members}`);
});
```
